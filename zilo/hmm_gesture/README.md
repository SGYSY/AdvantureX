...
## 接入 Wingman：iMessage → 网页来电 → MP3

Zilo 识别器已经接入本项目的本机硬件网关。默认使用预训练的
`打响指-hmm` 模型作为救场动作：识别成功后立刻通过 Photon 发送固定
iMessage，10 秒后让手机网页进入来电状态，滑动接听后播放 MP3。

先启动 Spectrum 和 Wingman 后端，再扫描并启动戒指：
...
python recognize.py \
  ...
  --ring \
  --address <戒指 UUID> \
  --trigger-gestures 甩-hmm
```

测试任意预训练动作时可以使用 `--trigger-gestures all`。识别器只访问
`127.0.0.1:8000`，不会把 BLE 标识或 Wingman 凭据暴露到公网。服务端
保留 30 秒防重复限制。

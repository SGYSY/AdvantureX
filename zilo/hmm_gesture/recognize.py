...
def parse_trigger_gestures(value: str) -> set[str]:
    """Parse a comma-separated model-name allowlist."""
    return {item.strip() for item in value.split(",") if item.strip()}


def post_wingman_gesture(
    url: str,
    gesture: str,
    confidence: float,
    device_id: str,
) -> dict[str, object]:
    """POST one recognized gesture to the local Wingman hardware gateway."""
    body = json.dumps(
        {
            "schema": "zilo-ring/v1",
            "gesture": gesture,
...
async def run_realtime(model_dir: Path, address: str, sample_rate: float, cutoff_hz: float,
                      window_size: int, window_overlap: int, wingman_url: str,
                      trigger_gestures: set[str], wingman_min_confidence: float) -> None:
            ...
            while True:
                try:
                    ...
                    if result:
                        ...
                        should_trigger = (
                            conf >= wingman_min_confidence
                            and ("all" in trigger_gestures or name in trigger_gestures)
                        )
                        if wingman_url and should_trigger:
                            try:
                                response = await asyncio.to_thread(
                                    post_wingman_gesture,
                                    wingman_url,
                                    name,
                                    conf,
                                    address,
                                ...
                                if response.get("accepted"):
                                    print("           → Wingman 已接收：正在发送 iMessage，10 秒后显示来电")
                                else:
                                    print(f"           → Wingman 未触发：{response.get('reason', '未知原因')}")
                            except Exception as exc:
                                print(f"           → Wingman 连接失败：{exc}")
...
def main():
    ...
    parser.add_argument(
        "--wingman-url",
        default="http://127.0.0.1:8000/api/v1/hardware/zilo",
        help="本机 Wingman 手势入口；传空字符串可关闭转发",
    )
    parser.add_argument(
        "--trigger-gestures",
        default="打响指-hmm",
        help="触发 Wingman 的模型名，逗号分隔；all 表示任意手势",
    )
    parser.add_argument(
        "--wingman-min-confidence",
        type=float,
        default=0.85,
        help="发送给 Wingman 的最低识别置信度",
    )
    ...
    if args.ring:
        ...
        asyncio.run(run_realtime(model_dir, args.address, args.sample_rate, args.cutoff_hz,
                                 args.window_size, args.window_overlap, args.wingman_url,
                                 parse_trigger_gestures(args.trigger_gestures),
                                 args.wingman_min_confidence))
    elif args.input:
